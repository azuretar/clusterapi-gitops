...
if [[ -n $1 ]]; #if not null
then 
    export CAPI_WORKLOAD_CLUSTER_NAME=$1 # Name of the CAPI workload cluster. Must consist of lower case alphanumeric characters, '-' or '.', and must start and end with an alphanumeric character (e.g. 'example.com', regex used for validation is '[a-z0-9]([-a-z0-9]*[a-z0-9])?(\.[a-z0-9]([-a-z0-9]*[a-z0-9])?)*')
else
    export CAPI_WORKLOAD_CLUSTER_NAME="capi-quickstart-1"
fi

# Install Control plane flag (true or false)
if [[ -n $2 ]]; #if not null
then 
    install_control_plane=$2
else
    install_control_plane=false
fi

## Set these variables here or in your ~/.bashrc file 
